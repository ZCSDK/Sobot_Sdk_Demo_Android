...
public final class PDF417Reader implements Reader, MultipleBarcodeReader {
  ...
  private static int getMaxCodewordWidth(ResultPoint[] p) {
    return Math.max(
        Math.max(getMaxWidth(p[0], p[4]), getMaxWidth(p[6], p[2]) * com.sobot.chat.widget.zxing.pdf417.PDF417Common.MODULES_IN_CODEWORD /
            com.sobot.chat.widget.zxing.pdf417.PDF417Common.MODULES_IN_STOP_PATTERN),
        Math.max(getMaxWidth(p[1], p[5]), getMaxWidth(p[7], p[3]) * com.sobot.chat.widget.zxing.pdf417.PDF417Common.MODULES_IN_CODEWORD /
            com.sobot.chat.widget.zxing.pdf417.PDF417Common.MODULES_IN_STOP_PATTERN));
  }

  private static int getMinCodewordWidth(ResultPoint[] p) {
    return Math.min(
        Math.min(getMinWidth(p[0], p[4]), getMinWidth(p[6], p[2]) * com.sobot.chat.widget.zxing.pdf417.PDF417Common.MODULES_IN_CODEWORD /
            com.sobot.chat.widget.zxing.pdf417.PDF417Common.MODULES_IN_STOP_PATTERN),
        Math.min(getMinWidth(p[1], p[5]), getMinWidth(p[7], p[3]) * com.sobot.chat.widget.zxing.pdf417.PDF417Common.MODULES_IN_CODEWORD /
