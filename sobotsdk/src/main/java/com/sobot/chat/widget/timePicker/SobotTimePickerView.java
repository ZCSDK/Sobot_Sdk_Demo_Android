...
public class SobotTimePickerView extends SobotBasePickerView implements View.OnClickListener {
    ...
    private SobotWheelView.DividerType dividerType;//分隔线类型
    ...
    public static class Builder {
        ...
        public Builder setLayoutRes(String res, SobotCustomListener customListener) {
            this.layoutRes = res;
    ...
    private void initView(Context context) {
        ...
        if (customListener == null) {
            LayoutInflater.from(context).inflate(ResourceUtils.getIdByName(context,"layout","sobot_pickerview_time"), contentContainer);

            //顶部标题
            tvTitle = (TextView) findViewById(ResourceUtils.getIdByName(context,"id","tvTitle"));
...
//            btnCancel.setTextSize(Size_Submit_Cancel);
            ...
            customListener.customLayout(LayoutInflater.from(context).inflate(ResourceUtils.getIdByName(context,"layout",layoutRes), contentContainer));
        }
        // 时间转轮 自定义控件
        LinearLayout timePickerView = (LinearLayout) findViewById(ResourceUtils.getIdByName(context,"id","timepicker"));
    ...
    public void returnData() {
        if (timeSelectListener != null) {
            try {
                Date date = SobotWheelTime.dateFormat.parse(wheelTime.getTime());
